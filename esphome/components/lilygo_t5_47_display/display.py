...
async def to_code(config):
    ...
    # Let op: NIET ook cg.register_component() aanroepen. register_display doet
    # dat al; de upstream-versie deed het dubbel en faalde op de validatie.
    await display.register_display(var, config)

    if CONF_LAMBDA in config:
        # Modern ESPHome heeft geen DisplayBufferRef meer; het is DisplayRef.
    ...
    # De git-URL hoort in het 'repository'-argument, niet in 'name'. Anders komt
    # er een kale URL in lib_deps te staan en vindt de build de headers niet.
